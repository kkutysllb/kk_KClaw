"""ACP agent server — exposes KClaw Agent via the Agent Client Protocol."""
...
class KClawACPAgent(acp.Agent):
    """ACP Agent implementation wrapping KClaw AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show KClaw version",
        },
    )
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate KClaw using the currently configured {provider} runtime credentials.",
                )
            ]
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run KClaw on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when KClaw has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
