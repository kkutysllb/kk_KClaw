...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.kclaw/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KCLAW_HOME", Path.home() / ".kclaw")) / "skills" / "red-teaming" / "godmode"
...
KCLAW_HOME = Path(os.getenv("KCLAW_HOME", Path.home() / ".kclaw"))
CONFIG_PATH = KCLAW_HOME / "config.yaml"
PREFILL_PATH = KCLAW_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/KClaw models — already uncensored, just needs clean prompt
    "kclaw": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "kclaw" in model_lower or "nous" in model_lower:
        return "kclaw"
...
def _get_current_model() -> tuple:
    """Read current model and provider from KClaw config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.kclaw/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart KClaw for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart KClaw for changes to take effect.")
