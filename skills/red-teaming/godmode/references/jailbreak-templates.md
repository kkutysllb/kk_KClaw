...
## 5. GODMODE FAST — KClaw 4 405B (Zero Refusal)

**Model:** `nousresearch/kclaw-4-405b`
**Strategy:** KClaw models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with KClaw Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.kclaw/config.yaml`:
