...
```bash
KCLAW_HOME="${KCLAW_HOME:-$HOME/.kclaw}"
GWORKSPACE_SKILL_DIR="$KCLAW_HOME/skills/productivity/google-workspace"
PYTHON_BIN="${KCLAW_PYTHON:-python3}"
if [ -x "$KCLAW_HOME/kclaw/venv/bin/python" ]; then
  PYTHON_BIN="$KCLAW_HOME/kclaw/venv/bin/python"
fi
...
- Token is stored at `google_token.json` under the active profile's `KCLAW_HOME` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `google_oauth_pending.json` under the active profile's `KCLAW_HOME` until exchange completes.
- KClaw now refuses to overwrite a full Google Workspace token with a narrower re-auth token missing Gmail scopes, so one profile's partial consent cannot silently break email actions later.
