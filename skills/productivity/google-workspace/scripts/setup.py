#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for KClaw Agent.
...
try:
    from kclaw_constants import display_kclaw_home, get_kclaw_home
except ModuleNotFoundError:
    KCLAW_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if KCLAW_AGENT_ROOT.exists():
        sys.path.insert(0, str(KCLAW_AGENT_ROOT))
    from kclaw_constants import display_kclaw_home, get_kclaw_home

KCLAW_HOME = get_kclaw_home()
TOKEN_PATH = KCLAW_HOME / "google_token.json"
CLIENT_SECRET_PATH = KCLAW_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KCLAW_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same KClaw profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to KClaw home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kclaw_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for KClaw")
