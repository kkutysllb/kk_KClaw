#!/usr/bin/env python3
"""Google Workspace API CLI for KClaw Agent.
...
def get_credentials():
    ...
    if missing_scopes:
        ...
        print(
            f"Re-run setup.py from the active KClaw profile ({display_kclaw_home()}) to restore full access.",
            file=sys.stderr,
        )
        sys.exit(1)
    return creds
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for KClaw Agent")
