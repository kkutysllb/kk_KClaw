...
def resolve_config_path() -> Path:
    ...
    default_path = Path.home() / ".kclaw" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into KClaw system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.kclaw
    ...
    def from_global_config(
        ...
        # A hosts.kclaw block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        # per-session: inherit KClaw session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'kclaw honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            kclaw_cfg = load_config()
            honcho_cfg = kclaw_cfg.get("honcho", {})
