...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'kclaw honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'kclaw honcho setup' first.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives KClaw persistent cross-session memory.")
    ...
    kclaw_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'kclaw honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = kclaw_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        kclaw_host["peerName"] = new_peer

    current_ai = kclaw_host.get("aiPeer") or cfg.get("aiPeer", "kclaw")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        kclaw_host["aiPeer"] = new_ai

    current_workspace = kclaw_host.get("workspace") or cfg.get("workspace", "kclaw")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        kclaw_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = kclaw_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        kclaw_host["observationMode"] = new_obs
    else:
        kclaw_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(kclaw_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        kclaw_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        kclaw_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = kclaw_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        kclaw_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = kclaw_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        kclaw_host["sessionStrategy"] = new_strat

    kclaw_host["enabled"] = True
    kclaw_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        kclaw_config = load_config()
        kclaw_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(kclaw_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: kclaw config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    kclaw honcho status     -- show full config")
    print("    kclaw honcho mode       -- change recall/observation mode")
    print("    kclaw honcho tokens     -- tune context and dialectic budgets")
    print("    kclaw honcho peer       -- update peer names")
    print("    kclaw honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: kclaw honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'kclaw honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: kclaw honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        user = kclaw.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = kclaw.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = kclaw.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = kclaw.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    KClaw' identity in Honcho. Seed with 'kclaw honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: kclaw honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = kclaw.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = kclaw.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = kclaw.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. KClaw asks Honcho's AI peer a question")
        ...
        print("\n  Set with: kclaw honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'kclaw honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    kclaw honcho identity --show        — show both peer representations")
        print("    kclaw honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    print("\nHoncho migration: OpenClaw native memory → KClaw\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives KClaw persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  kclaw honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'kclaw honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'kclaw honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  kclaw honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'kclaw'")
        print("  with Honcho configured and no prior session history.")
        print("  (KClaw calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    kclaw honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'kclaw honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In KClaw, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'kclaw honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    kclaw honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  kclaw honcho identity <path/to/SOUL.md>")
    ...
    print("    KClaw:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    KClaw:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    KClaw:   Honcho observes every message and updates representations")
    ...
    print("    KClaw:   per-session by default — each run gets its own session")
    print("              Map a custom name:  kclaw honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. kclaw honcho setup              — configure API key (required)")
        print("  2. kclaw honcho migrate            — re-run this walkthrough")
    else:
        print("  1. kclaw honcho status             — verify Honcho connection")
        print("  2. kclaw                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. kclaw honcho identity --show    — verify AI peer representation")
        print("  4. kclaw honcho tokens             — tune context and dialectic budgets")
        print("  5. kclaw honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'kclaw memory setup'...\n")
        from kclaw_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    ...
    *subparser* 是 ``kclaw honcho`` 的解析器。
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to kclaw memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to KClaw Honcho",
    )
