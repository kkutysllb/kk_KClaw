...
def _load_supermemory_config(kclaw_home: str) -> dict:
    config = _default_config()
    config_path = Path(kclaw_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, kclaw_home: str) -> None:
    config_path = Path(kclaw_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, kclaw_home):
        ...
        _save_supermemory_config(sanitized, kclaw_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from kclaw_constants import get_kclaw_home
        self._kclaw_home = kwargs.get("kclaw_home") or str(get_kclaw_home())
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "kclaw", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "kclaw_memory", "target": target, "type": "explicit_memory"},
