...
Or via $KCLAW_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    profile_path = get_kclaw_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "kclaw": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "kclaw"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between KClaw Agent and the User"
    ...
    def post_setup(self, kclaw_home: str, config: dict) -> None:
        ...
        from kclaw_cli.memory_setup import _curses_select
        ...
        provider_config["bank_id"] = "kclaw"
        provider_config["recall_budget"] = "mid"
        bank_id = "kclaw"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, kclaw_home)

        if env_writes:
            env_path = Path(kclaw_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "kclaw"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between KClaw Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "kclaw"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "kclaw"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("kclaw", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "kclaw")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between KClaw Agent and the User")
