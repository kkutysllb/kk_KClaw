...
if tinker_atropos_dir.exists():
    ...
    os.environ['KCLAW_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to kclaw directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.kclaw/config.yaml if not provided)
    ...
    """
    # Load config from ~/.kclaw/config.yaml
