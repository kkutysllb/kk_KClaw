...
- Path resolution (absolute, relative to KCLAW_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp KCLAW_HOME."""
    kclaw_home = tmp_path / ".kclaw"
    kclaw_home.mkdir()
    (kclaw_home / "cron").mkdir()
    (kclaw_home / "cron" / "output").mkdir()
    (kclaw_home / "scripts").mkdir()
    monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "KCLAW_DIR", kclaw_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", kclaw_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", kclaw_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", kclaw_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.kclaw/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("KCLAW_SESSION_PLATFORM") is None
        assert os.environ.get("KCLAW_SESSION_CHAT_ID") is None
        assert os.environ.get("KCLAW_SESSION_CHAT_NAME") is None
