...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._kclaw_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_kclaw_bin(self):
        """After _INSTALL_FAILED, manual install in KCLAW_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        kclaw_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(kclaw_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(kclaw_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._kclaw_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == kclaw_bin
            assert _tirith_mod._resolved_path == kclaw_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# KCLAW_HOME isolation
# ---------------------------------------------------------------------------

class TestKClawHomeIsolation:
    def test_kclaw_bin_dir_respects_kclaw_home(self):
        """_kclaw_bin_dir must use KCLAW_HOME, not hardcoded ~/.kclaw."""
        from tools.tirith_security import _kclaw_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"KCLAW_HOME": tmpdir}):
            result = _kclaw_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_kclaw_home(self):
        """_failure_marker_path must use KCLAW_HOME, not hardcoded ~/.kclaw."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"KCLAW_HOME": "/custom/kclaw"}):
            result = _failure_marker_path()
        assert result == "/custom/kclaw/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets KCLAW_HOME; verify it's active."""
        kclaw_home = os.getenv("KCLAW_HOME")
        assert kclaw_home is not None, "KCLAW_HOME should be set by conftest"
        assert "kclaw_test" in kclaw_home, "Should point to test temp dir"

    def test_get_kclaw_home_fallback(self):
        """Without KCLAW_HOME set, falls back to ~/.kclaw."""
        from tools.tirith_security import _get_kclaw_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove KCLAW_HOME entirely
            os.environ.pop("KCLAW_HOME", None)
            result = _get_kclaw_home()
        assert result == os.path.join(os.path.expanduser("~"), ".kclaw")
