...
def _restore_tool_modules():
    original_kclaw_home = os.environ.get("KCLAW_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_kclaw_home is None:
            os.environ.pop("KCLAW_HOME", None)
        else:
            os.environ["KCLAW_HOME"] = original_kclaw_home
        _reset_modules(("tools", "kclaw_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kclaw_cli = types.ModuleType("kclaw_cli")
    kclaw_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kclaw_cli"] = kclaw_cli
    kclaw_home = tmp_path / "kclaw-home"
    os.environ["KCLAW_HOME"] = str(kclaw_home)
    sys.modules["kclaw_cli.config"] = types.SimpleNamespace(
        get_kclaw_home=lambda: kclaw_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kclaw", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": kclaw_home / "modal_snapshots.json",
