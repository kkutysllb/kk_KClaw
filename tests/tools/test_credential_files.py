...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        kclaw_home.mkdir()
        (kclaw_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"KCLAW_HOME": str(kclaw_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(kclaw_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.kclaw/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        (kclaw_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        (kclaw_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        (kclaw_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.kclaw/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        kclaw_home = tmp_path / ".kclaw"
        (kclaw_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"KCLAW_HOME": str(kclaw_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.kclaw")

        assert mounts[0]["container_path"] == "/home/user/.kclaw/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.kclaw/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.kclaw/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape KCLAW_HOME."""
        monkeypatch.setenv("KCLAW_HOME", str(tmp_path / ".kclaw"))
        (tmp_path / ".kclaw").mkdir()

        # Create a sensitive file one level above kclaw_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside kclaw_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside KCLAW_HOME must still be registered."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))
        (kclaw_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_kclaw_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of KCLAW_HOME must be allowed."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        subdir = kclaw_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside KCLAW_HOME pointing outside must be rejected."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))

        # Create a sensitive file outside kclaw_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside kclaw_home pointing outside
        symlink = kclaw_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes KCLAW_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, kclaw_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape KCLAW_HOME."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))
        ...
        sensitive.write_text("{}")
        self._write_config(kclaw_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(kclaw_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside KCLAW_HOME via config must still mount."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))

        (kclaw_home / "oauth.json").write_text("{}")
        self._write_config(kclaw_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        (kclaw_home / "cache" / "documents").mkdir(parents=True)
        (kclaw_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.kclaw/cache/documents" in paths
        assert "/root/.kclaw/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.kclaw/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        # Use legacy dir name — get_kclaw_dir prefers old if it exists
        (kclaw_home / "document_cache").mkdir()
        (kclaw_home / "image_cache").mkdir()
        monkeypatch.setenv("KCLAW_HOME", str(kclaw_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(kclaw_home / "document_cache") in host_paths
        assert str(kclaw_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.kclaw/cache/documents" in container_paths
        assert "/root/.kclaw/cache/images" in container_paths

    def test_empty_kclaw_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        kclaw_home = tmp_path / ".kclaw"
        doc_dir = kclaw_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        kclaw_home = tmp_path / ".kclaw"
        ss_dir = kclaw_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.kclaw/cache/screenshots/session_abc/screen1.png"
