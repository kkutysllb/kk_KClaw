...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/kclaw-snap-test123.sh"
    env._cwd_file = "/tmp/kclaw-cwd-test123.txt"
    env._cwd_marker = "__KCLAW_CWD_test123__"
...
def test_init_env_args_uses_kclaw_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_kclaw_env_vars", lambda: {"GITHUB_TOKEN": "value_from_dotenv"})
...
def test_init_env_args_prefers_shell_env_over_kclaw_dotenv(monkeypatch):
...
def test_forward_env_overrides_docker_env_in_init_args(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_kclaw_env_vars", lambda: {})
