...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "KCLAW_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["KCLAW_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_kclaw_home_override(self, tmp_path):
        """Auth lookup should read from KCLAW_HOME/auth.json, not ~/.kclaw/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".kclaw").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``kclaw tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "KCLAW_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
