...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "KClaw Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_kclaw_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("KCLAW_HOME", str(tmp_path / ".kclaw"))

    kclaw_env = tmp_path / ".kclaw" / ".env"
    kclaw_env.parent.mkdir(parents=True)
    kclaw_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.kclaw/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
