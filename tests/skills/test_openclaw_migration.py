...
"""
    ...
    assert "Tyler Williams > Active Projects: KClaw Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `kclaw cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # kclaw_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.kclaw/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "kclaw_config_mod"}
