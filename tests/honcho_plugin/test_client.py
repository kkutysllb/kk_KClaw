...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "kclaw"
        assert config.workspace_id == "kclaw"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"kclaw": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"kclaw": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"kclaw": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kclaw"
        ):
            result = config.resolve_session_name("/home/user/kclaw/subdir")
        assert result == "kclaw"
...
class TestResolveConfigPath:
    def test_prefers_kclaw_home_when_exists(self, tmp_path):
        kclaw_home = tmp_path / "kclaw"
        kclaw_home.mkdir()
        local_cfg = kclaw_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        kclaw_home = tmp_path / "kclaw"
        kclaw_home.mkdir()
        # No honcho.json in KCLAW_HOME — also isolate ~/.kclaw so
    ...
    def test_falls_back_to_global_without_kclaw_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("KCLAW_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_kclaw(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("KCLAW_HONCHO_HOST", None)
            os.environ.pop("KCLAW_HOME", None)
            assert resolve_active_host() == "kclaw"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"KCLAW_HONCHO_HOST": "kclaw.coder"}):
            assert resolve_active_host() == "kclaw.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KCLAW_HONCHO_HOST", None)
            with patch("kclaw_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "kclaw.coder"

    def test_default_profile_returns_kclaw(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KCLAW_HONCHO_HOST", None)
            with patch("kclaw_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "kclaw"

    def test_custom_profile_returns_kclaw(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KCLAW_HONCHO_HOST", None)
            with patch("kclaw_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "kclaw"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KCLAW_HONCHO_HOST", None)
            # Temporarily remove kclaw_cli.profiles to simulate import failure
            saved = sys.modules.get("kclaw_cli.profiles")
            sys.modules["kclaw_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kclaw"
            finally:
                if saved is not None:
                    sys.modules["kclaw_cli.profiles"] = saved
                else:
                    sys.modules.pop("kclaw_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kclaw.coder")
        assert config.host == "kclaw.coder"
        assert config.workspace_id == "kclaw"  # shared workspace
        assert config.ai_peer == "kclaw.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kclaw")
        assert config.host == "kclaw"
        assert config.workspace_id == "kclaw"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "kclaw": {"aiPeer": "kclaw", "peerName": "alice"},
                "kclaw.coder": {
                    "aiPeer": "kclaw.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="kclaw.coder", config_path=config_file,
        )
        assert config.host == "kclaw.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kclaw.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="kclaw.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "kclaw.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kclaw": {"enabled": True, "aiPeer": "kclaw"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kclaw": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
