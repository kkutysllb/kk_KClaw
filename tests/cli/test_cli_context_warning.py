...
def _isolate(tmp_path, monkeypatch):
    """Isolate KCLAW_HOME so tests don't touch real config."""
    home = tmp_path / ".kclaw"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal KClawCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = KClawCLI.__new__(KClawCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["kclaw-core"]
        obj.compact = False
