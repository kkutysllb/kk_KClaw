...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("kclaw/kclaw-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned kclaw/* and pr-* branches."""

    def test_prunes_orphaned_kclaw_branch(self, git_repo):
        """kclaw/kclaw-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "kclaw/kclaw-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "kclaw/kclaw-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "kclaw/kclaw-deadbeef" in result.stdout
        ...
        assert "kclaw/kclaw-deadbeef" in orphaned
        ...
        assert "kclaw/kclaw-deadbeef" not in result.stdout
