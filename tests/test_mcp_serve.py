"""
Tests for mcp_serve — KClaw MCP server.
...
def _isolate_kclaw_home(tmp_path, monkeypatch):
    """Redirect KCLAW_HOME to a temp directory."""
    monkeypatch.setenv("KCLAW_HOME", str(tmp_path))
    try:
        import kclaw_constants
        monkeypatch.setattr(kclaw_constants, "get_kclaw_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking kclaw_state schema."""
