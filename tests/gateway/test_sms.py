...
class TestSmsToolset:
    def test_kclaw_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("kclaw-sms")
        assert ts is not None
        assert "tools" in ts

    def test_kclaw_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("kclaw-gateway")
        assert gw is not None
        assert "kclaw-sms" in gw["includes"]
