...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @kclaw:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("kclaw can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("KCLAW can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@kclaw:example.org">KClaw</a> help'
        assert self.adapter._is_bot_mentioned("KClaw help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "kclawbot" should not match word-boundary check for "kclaw"
        assert not self.adapter._is_bot_mentioned("kclawbot is here")
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@kclaw:example.org help me")
        assert result == "help me"

    def test_strip_localpart(self):
        result = self.adapter._strip_mention("kclaw help me")
        assert result == "help me"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@kclaw:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    event = _make_event("@kclaw:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    formatted = '<a href="https://matrix.to/#/@kclaw:example.org">KClaw</a> help'
    event = _make_event("KClaw help", formatted_body=formatted)
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    event = _make_event("@kclaw:example.org")
