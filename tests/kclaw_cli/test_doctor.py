...
import kclaw_cli.doctor as doctor
import kclaw_cli.gateway as gateway_cli
from kclaw_cli import doctor as doctor_mod
from kclaw_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "KCLAW_HOME", kclaw_home)
    monkeypatch.delenv("KCLAW_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("KCLAW_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_kclaw_home(self, tmp_path, provider=""):
        """Create a minimal KCLAW_HOME with config.yaml."""
        home = tmp_path / ".kclaw"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_kclaw_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "KCLAW_HOME", home)
        ...
        try:
            from kclaw_cli import auth as _auth_mod
