"""Tests for the update check mechanism in kclaw_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from kclaw_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("kclaw_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KCLAW_HOME", str(tmp_path))
    with patch("kclaw_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import kclaw_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "kclaw_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when KCLAW_HOME has no git repo."""
    import kclaw_cli.banner as banner
    ...
    # Point KCLAW_HOME at a temp dir with no kclaw/.git
    monkeypatch.setenv("KCLAW_HOME", str(tmp_path))
    with patch("kclaw_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from kclaw_cli.main import _invalidate_update_cache

    # Build a fake ~/.kclaw with default + two named profiles
