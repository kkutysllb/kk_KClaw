"""Tests for the KClaw plugin system (kclaw_cli.plugins)."""
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.kclaw/plugins/ are discovered."""
        plugins_dir = tmp_path / "kclaw_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("KCLAW_HOME", str(tmp_path / "kclaw_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KCLAW_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".kclaw" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under kclaw_plugins.<name>."""
        plugins_dir = tmp_path / "kclaw_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("KCLAW_HOME", str(tmp_path / "kclaw_test"))

        # Clean up any prior namespace module
        sys.modules.pop("kclaw_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "kclaw_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kclaw_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import kclaw_cli.plugins as plugins_mod
...
# in PluginContext (kclaw_cli/plugins.py).  The tests referenced _plugin_commands,
