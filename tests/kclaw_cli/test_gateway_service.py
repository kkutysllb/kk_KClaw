...
import kclaw_cli.gateway as gateway_cli


class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "kclaw-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.kclaw.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.kclaw.gateway"])
            ),
        )
...
class TestSystemUnitKClawHome:
    """KCLAW_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("KCLAW_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'KCLAW_HOME=/home/alice/.kclaw' in unit
        assert '/root/.kclaw' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: KCLAW_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KCLAW_HOME", "/root/.kclaw/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'KCLAW_HOME=/home/alice/.kclaw/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_kclaw_home(self, monkeypatch):
        # Custom KCLAW_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KCLAW_HOME", "/opt/kclaw-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'KCLAW_HOME=/opt/kclaw-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's KCLAW_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        kclaw_home = str(gateway_cli.get_kclaw_home().resolve())
        assert f'KCLAW_HOME={kclaw_home}' in unit


class TestKClawHomeForTargetUser:
    """Unit tests for _kclaw_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("KCLAW_HOME", raising=False)

        result = gateway_cli._kclaw_home_for_target_user("/home/alice")
        assert result == "/home/alice/.kclaw"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._kclaw_home_for_target_user("/home/alice")
        assert result == "/home/alice/.kclaw/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KCLAW_HOME", "/opt/kclaw")

        result = gateway_cli._kclaw_home_for_target_user("/home/alice")
        assert result == "/opt/kclaw"
...
class TestProfileArg:
    ...
    def test_default_kclaw_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.kclaw should not produce a --profile flag."""
        kclaw_home = tmp_path / ".kclaw"
        kclaw_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        result = gateway_cli._profile_arg(str(kclaw_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.kclaw/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".kclaw" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile KCLAW_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "kclaw"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.kclaw/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".kclaw" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".kclaw" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KCLAW_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_kclaw_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
