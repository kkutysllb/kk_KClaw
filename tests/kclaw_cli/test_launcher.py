"""Tests for the top-level `./kclaw` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kclaw` should use `kclaw_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kclaw"
    called = []

    fake_main_module = types.ModuleType("kclaw_cli.main")

    def fake_main():
        called.append("kclaw_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kclaw_cli.main", fake_main_module)
