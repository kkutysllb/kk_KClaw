...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      kclaw = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "kclaw-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${kclawVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "kclaw-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${kclaw}/bin/kclaw || (echo "FAIL: kclaw binary missing"; exit 1)
          test -x ${kclaw}/bin/kclaw || (echo "FAIL: kclaw binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${kclaw}/bin/kclaw version 2>&1 | grep -qi "kclaw" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "kclaw-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in kclaw kclaw kclaw-acp; do
            test -x ${kclaw}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "kclaw-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking kclaw --help ==="
          ${kclaw}/bin/kclaw --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${kclaw}/bin/kclaw --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "kclaw-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${kclaw}/share/kclaw/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${kclaw}/share/kclaw/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "KCLAW_BUNDLED_SKILLS" ${kclaw}/bin/kclaw || \
            (echo "FAIL: KCLAW_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: KCLAW_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify KCLAW_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "kclaw-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(KCLAW_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking KCLAW_MANAGED guards ==="
          check_blocked "config set" ${kclaw}/bin/kclaw config set model foo
          check_blocked "config edit" ${kclaw}/bin/kclaw config edit
        ...
        in pkgs.runCommand "kclaw-config-roundtrip" {
          ...
          merge_and_load() {
            local kclaw_home="$1"
            export KCLAW_HOME="$kclaw_home"
            ${configMergeScript} ${nixSettings} "$kclaw_home/config.yaml"
            ${kclawVenv}/bin/python3 -c '
import json, sys
