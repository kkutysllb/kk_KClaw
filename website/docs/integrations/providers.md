...
This page covers setting up inference providers for KClaw Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use KClaw.

## Inference Providers

You need at least one way to connect to an LLM. Use `kclaw model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `kclaw model` (OAuth, subscription-based) |
| **OpenAI Codex** | `kclaw model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `kclaw model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `kclaw model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `kclaw model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.kclaw/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.kclaw/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.kclaw/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.kclaw/.env` (provider: `kimi-coding`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.kclaw/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.kclaw/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.kclaw/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.kclaw/.env` (provider: `kilocode`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.kclaw/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.kclaw/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.kclaw/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.kclaw/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.kclaw/.env` (provider: `gemini`) |
| **Custom Endpoint** | `kclaw model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). KClaw stores the resulting credentials in its own auth store under `~/.kclaw/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
kclaw chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `kclaw model`
# KClaw will use Claude Code's credential store directly when available
kclaw model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
kclaw chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
kclaw chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `kclaw model`, KClaw prefers Claude Code's own credential store over copying the token into `~/.kclaw/.env`. That keeps refreshable Claude credentials refreshable.
...
KClaw supports GitHub Copilot as a first-class provider with two modes:
...
```bash
kclaw chat --provider copilot --model gpt-5.4
```
...
If no token is found, `kclaw model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `kclaw model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `kclaw model` to authenticate via OAuth instead.
:::
...
```bash
kclaw chat --provider copilot-acp --model copilot-acp
...
| `KCLAW_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `KCLAW_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
kclaw chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.kclaw/.env

# Kimi / Moonshot AI
kclaw chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.kclaw/.env

# MiniMax (global endpoint)
kclaw chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.kclaw/.env

# MiniMax (China endpoint)
kclaw chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.kclaw/.env

# Alibaba Cloud / DashScope (Qwen models)
kclaw chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.kclaw/.env
```
...
When using the Z.AI / GLM provider, KClaw automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) Prompt Caching

When using xAI as a provider (any base URL containing `x.ai`), KClaw automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
kclaw chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.kclaw/.env

# Short alias
kclaw chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
KClaw Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point KClaw at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of KClaw — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `kclaw setup` or config migration. Use `kclaw model` or edit `config.yaml` directly.
:::
...
When switching providers, KClaw persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like KClaw.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in KClaw) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `kclaw` (Qwen 2.5, KClaw 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure KClaw to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but KClaw won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), KClaw 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, KClaw) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since KClaw Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your KClaw config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your KClaw config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with KClaw.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running KClaw inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser kclaw` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. KClaw's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what KClaw thinks the context is
...
KClaw auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), KClaw uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low `max_tokens` on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure KClaw with `model.max_tokens` in config.yaml.
2. **Context exhaustion** — The model filled its context window. Increase context length or enable [context compression](/docs/user-guide/configuration#context-compression) in KClaw.
...
Then configure KClaw with `kclaw model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure KClaw with `kclaw model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `kclaw model` → Custom endpoint, or in `config.yaml`:
...
KClaw uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`kclaw model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — KClaw uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `kclaw model` menu.
...
:::tip
You can switch between providers at any time with `kclaw model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point KClaw at your instance (no API key needed):
   ```bash
   kclaw config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.kclaw/config.yaml`:
...
Configure a backup provider:model that KClaw switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
Optional cheap-vs-strong routing lets KClaw keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, KClaw may route it to `cheap_model`
- If the turn looks complex, KClaw stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, KClaw falls back to the primary model automatically
