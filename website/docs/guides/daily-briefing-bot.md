...
2. **KClaw spins up** a fresh agent session with your prompt
...
- **KClaw Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.kclaw/cron/output/` and you can read them anytime.
:::
...
KClaw will search the web, read through results, and produce something like:
...
KClaw will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell KClaw to delegate each topic to a sub-agent:
...
KClaw will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
