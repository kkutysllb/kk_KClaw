---
sidebar_position: 7
title: "Use SOUL.md with KClaw"
description: "How to use SOUL.md to shape KClaw Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with KClaw

`SOUL.md` is the **primary identity** for your KClaw instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want KClaw to feel like the same assistant every time you talk to it — or if you want to replace the KClaw persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm KClaw should be
- what KClaw should avoid stylistically
- how KClaw should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who KClaw is and how KClaw speaks
...
KClaw now uses only the global SOUL file for the current instance:

```text
~/.kclaw/SOUL.md
```

If you run KClaw with a custom home directory, it becomes:
...
KClaw automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, KClaw does not overwrite it
- if the file exists but is empty, KClaw adds nothing from it to the prompt

## How KClaw uses it

When KClaw starts a session, it reads `SOUL.md` from `KCLAW_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how KClaw feels.
...
KClaw already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
Then restart KClaw or start a new session.
...
### I edited SOUL.md but KClaw still sounds the same

Check:
- you edited `~/.kclaw/SOUL.md` or `$KCLAW_HOME/SOUL.md`
...
### KClaw is ignoring parts of my SOUL.md
