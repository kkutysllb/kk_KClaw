...
description: "How KClaw stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `kclaw_cli/cron.py` | CLI `kclaw cron` subcommands |
...
Jobs are stored in `~/.kclaw/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `kclaw cron` commands are run or during active CLI sessions.
...
```python
# ~/.kclaw/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.kclaw/cron/output/` |
...
The `kclaw cron` CLI provides direct job management:

```bash
kclaw cron list                    # Show all jobs
kclaw cron create                  # Interactive job creation (alias: add)
kclaw cron edit <job_id>           # Edit job configuration
kclaw cron pause <job_id>          # Pause a running job
kclaw cron resume <job_id>         # Resume a paused job
kclaw cron run <job_id>            # Trigger immediate execution
kclaw cron remove <job_id>         # Delete a job
```
