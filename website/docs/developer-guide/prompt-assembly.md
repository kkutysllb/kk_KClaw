...
description: "How KClaw builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

KClaw deliberately separates:
...
1. agent identity — `SOUL.md` from `KCLAW_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.kclaw/SOUL.md)
You are KClaw, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.kclaw/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are KClaw Agent, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_kclaw_md(cwd_path)       # 1. .kclaw.md / KCLAW.md (walks to git root)
    ...
    # SOUL.md from KCLAW_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.kclaw.md`, `KCLAW.md` | CWD up to git root | KClaw-native project config |
...
- **YAML frontmatter stripped** — `.kclaw.md` frontmatter is removed (reserved for future config overrides)
...
1. `.kclaw.md` / `KCLAW.md` (walks to git root)
