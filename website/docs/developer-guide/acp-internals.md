...
The ACP adapter wraps KClaw' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
kclaw acp / kclaw-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.kclaw/.env
  -> configure stderr logging
  -> construct KClawACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> KClaw `once`
- `allow_always` -> KClaw `always`
- reject options -> KClaw `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps KClaw tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["kclaw-acp"])
...
Instead it reuses KClaw' runtime resolver:

- `acp_adapter/auth.py`
- `kclaw_cli/runtime_provider.py`

So ACP advertises and uses the currently configured KClaw provider/credentials.
...
- `toolsets.py` — `kclaw-acp` toolset definition
- `kclaw_cli/main.py` — `kclaw acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `kclaw-acp` script
