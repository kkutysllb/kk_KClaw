...
sidebar_position: 1
title: "Architecture"
description: "KClaw Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of KClaw Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ kclaw_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # KClawCLI — interactive terminal UI (~8,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── kclaw_state.py           # SQLite session/state database with FTS5
├── kclaw_constants.py       # KCLAW_HOME, profile-aware paths
...
│
├── kclaw_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `kclaw` subcommands (~5,500 lines)
...
│   ├── skills_config.py      # kclaw skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # kclaw tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # kclaw gateway start/stop
│
...
```text
User input → KClawCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .kclaw.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.kclaw/plugins/` (user), `.kclaw/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Memory providers are a specialized plugin type under `plugins/memory/`.

→ [Plugin Guide](/docs/guides/build-a-kclaw-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes KClaw as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`kclaw -p <name>`) gets its own KCLAW_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
