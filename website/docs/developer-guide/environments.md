...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the KClaw-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

KClaw Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for KClaw/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- KClawAgentBaseEnv
    KClawAgentBaseEnv <|-- TerminalTestEnv
    KClawAgentBaseEnv <|-- KClawSweEnv
    KClawAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The kclaw layer (`environments/kclaw_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls kclaw's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `KClawAgentLoop` and scores the result
...
Your environment inherits from `KClawAgentBaseEnv` and implements five methods:
...
`KClawAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as kclaw's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any kclaw tool |
...
parser = get_parser("kclaw")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `kclaw`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"kclaw"`, `"qwen"`, `"mistral"`)
...
### KClawAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which kclaw toolsets to enable |
...
| `tool_call_parser` | `str` | `"kclaw"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "kclaw[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "kclaw[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── kclaw_base_env.py          # Abstract base class (KClawAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (KClawAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── kclaw_parser.py        # KClaw/ChatML <tool_call> format
...
├── kclaw_swe_env/             # SWE-bench training environment
│
