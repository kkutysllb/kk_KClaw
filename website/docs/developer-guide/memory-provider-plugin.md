...
description: "How to build a memory provider plugin for KClaw Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give KClaw Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          kclaw_home (str): Active KCLAW_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `kclaw memory setup` | **Yes** |
| `save_config(values, kclaw_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `kclaw memory setup`:
...
Every field in `get_config_schema()` is prompted during `kclaw memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$KCLAW_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, kclaw_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(kclaw_home) / "my-provider.json"
...
All storage paths **must** use the `kclaw_home` kwarg from `initialize()`, not hardcoded `~/.kclaw`:

```python
# CORRECT — profile-scoped
from kclaw_constants import get_kclaw_home
data_dir = get_kclaw_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.kclaw/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `kclaw my-provider status`, `kclaw my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `kclaw <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `kclaw --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: kclaw my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the kclaw my-provider argparse tree.
