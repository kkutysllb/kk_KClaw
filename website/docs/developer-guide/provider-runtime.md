...
description: "How KClaw resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

KClaw has a shared provider runtime resolver used across:
...
- `kclaw_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `kclaw_cli/auth.py` — provider registry, `resolve_provider()`
- `kclaw_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
That ordering matters because KClaw treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `kclaw model`.
...
This resolver is the main reason KClaw can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.kclaw/.env` and run with `--provider ai-gateway`. KClaw fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

KClaw contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
KClaw also distinguishes between:
...
When provider resolution selects `anthropic`, KClaw uses:
...
- KClaw preflights Anthropic credential refresh before native Messages API calls
- KClaw still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, KClaw resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `kclaw model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

KClaw supports a configured fallback model/provider pair, allowing runtime failover when the primary model encounters errors.
