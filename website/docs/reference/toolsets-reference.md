...
description: "Reference for KClaw core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `kclaw-cli` is the default for interactive CLI sessions)
...
```bash
kclaw chat --toolsets web,file,terminal
kclaw chat --toolsets debugging        # composite — expands to file + terminal + web
kclaw chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - kclaw-cli          # default for CLI
  # - kclaw-telegram   # override for Telegram gateway
```

### Interactive management

```bash
kclaw tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call KClaw tools programmatically. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `kclaw-cli`:

| Toolset | Differences from `kclaw-cli` |
|---------|-------------------------------|
| `kclaw-cli` | Full toolset — all 38 tools including `clarify`. The default for interactive CLI sessions. |
| `kclaw-acp` | Drops `clarify`, `cronjob`, `image_generate`, `mixture_of_agents`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `kclaw-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `kclaw-telegram` | Same as `kclaw-cli`. |
| `kclaw-discord` | Same as `kclaw-cli`. |
| `kclaw-slack` | Same as `kclaw-cli`. |
| `kclaw-whatsapp` | Same as `kclaw-cli`. |
| `kclaw-signal` | Same as `kclaw-cli`. |
| `kclaw-matrix` | Same as `kclaw-cli`. |
| `kclaw-mattermost` | Same as `kclaw-cli`. |
| `kclaw-email` | Same as `kclaw-cli`. |
| `kclaw-sms` | Same as `kclaw-cli`. |
| `kclaw-dingtalk` | Same as `kclaw-cli`. |
| `kclaw-feishu` | Same as `kclaw-cli`. |
| `kclaw-wecom` | Same as `kclaw-cli`. |
| `kclaw-bluebubbles` | Same as `kclaw-cli`. |
| `kclaw-homeassistant` | Same as `kclaw-cli`. |
| `kclaw-webhook` | Same as `kclaw-cli`. |
| `kclaw-gateway` | Union of all messaging platform toolsets. Used internally when the gateway needs the broadest possible tool set. |
...
## Relationship to `kclaw tools`

The `kclaw tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
