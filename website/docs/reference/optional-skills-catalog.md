...
description: "Official optional skills shipped with kclaw — install via kclaw skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the kclaw repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
kclaw skills install official/blockchain/solana
kclaw skills install official/mlops/flash-attention
```
...
```bash
kclaw skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with KClaw — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from KClaw via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into KClaw Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **kclaw-atropos-environments** | Build, test, and debug KClaw Agent RL environments for Atropos training. Covers the KClawAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give KClaw phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
