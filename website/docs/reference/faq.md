...
description: "Frequently asked questions and solutions to common issues with KClaw Agent"
---
...
### What LLM providers work with KClaw?

KClaw Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — kkutysllb's own inference endpoint
...
Set your provider with `kclaw model` or by editing `~/.kclaw/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). KClaw Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.kclaw/`.

### Can I use it offline / with local models?

Yes. Run `kclaw model`, select **Custom endpoint**, and enter your server's URL:
...
KClaw persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in KClaw — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

KClaw Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same KClaw Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use KClaw programmatically:

```python
from run_agent import AIAgent

agent = AIAgent(model="openrouter/nous/kclaw-3-llama-3.1-70b")
...
#### `kclaw: command not found` after installation
...
**Cause:** KClaw requires Python 3.11 or newer.
...
# Or set directly
kclaw config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.kclaw/.env` for conflicting entries.
:::
...
# Set a valid model
kclaw config set KCLAW_MODEL openrouter/nous/kclaw-3-llama-3.1-70b

# Or specify per-session
kclaw chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `kclaw chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or KClaw detected the wrong context length for your model.
...
kclaw chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, KClaw may have the wrong context length for your model. Check what it detected:
...
**Cause:** KClaw detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — KClaw never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `kclaw chat`
...
cat ~/.kclaw/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `kclaw gateway setup`
- Check gateway logs: `cat ~/.kclaw/logs/gateway.log | tail -50`
...
Configure in `~/.kclaw/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "kclaw[telegram]"   # or [discord], [slack], [whatsapp]
...
**Solution:** The gateway captures your shell PATH when you run `kclaw gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
kclaw gateway install    # Re-snapshots your current PATH
kclaw gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.kclaw.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `kclaw chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `kclaw chat -t "terminal"`
...
cd ~/.kclaw/kclaw && uv pip install -e ".[mcp]"
...
Verify your `~/.kclaw/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
kclaw config show | grep -A 12 mcp_servers

# Restart KClaw or reload MCP after config changes
kclaw chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with KClaw](/docs/guides/use-mcp-with-kclaw)
...
:::warning
If an MCP server crashes mid-request, KClaw will report a timeout. Check the server's own logs (not just KClaw logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `kclaw profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `kclaw update`?

`kclaw update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `kclaw update` once — it covers every profile on the machine.
...
kclaw profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
kclaw profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.kclaw/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** KClaw can route subagents to a different model automatically. Set this in `~/.kclaw/config.yaml`:
...
Now when you tell KClaw "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can KClaw do this?

**Current limitation:** KClaw profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, KClaw reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `kclaw skills config` settings don't seem to take effect.

**Solution:** Use `kclaw skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`kclaw gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** KClaw creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting KClaw to another machine
...
2. Copy your entire `~/.kclaw/` directory **except** the `kclaw` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='kclaw' ~/.kclaw/ newmachine:~/.kclaw/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   kclaw profile export default ./kclaw-backup.tar.gz

   # On target machine
   kclaw profile import ./kclaw-backup.tar.gz default
   ```

3. On the new machine, run `kclaw setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.kclaw/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.kclaw/kclaw/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the KClaw installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a KClaw-specific issue — it's a shell config permissions problem.
...
kclaw config show | head -20

# Re-run model selection
kclaw model

# Or test with a known-good model
kclaw chat -q "hello" --model anthropic/claude-sonnet-4.6
```
