...
KClaw Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.kclaw/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.kclaw/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`kclaw` or `kclaw chat`) |
...
kclaw -c "my project"   # → resumes "my project #3"
```
...
kclaw --resume 20250305_091523_a1b2c3d4
kclaw -r 20250305_091523_a1b2c3d4

# Resume by title
kclaw --resume "refactoring auth"

# Or with the chat subcommand
kclaw chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `kclaw sessions list`.

### Conversation Recap on Resume

When you resume a session, KClaw displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a KClaw session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.kclaw/config.yaml`:
...
KClaw automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `kclaw sessions list` or `kclaw sessions browse`.
...
```bash
kclaw sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), KClaw creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`kclaw -c "my project"`), it automatically picks the most recent session in the lineage.
...
KClaw provides a full set of session management commands via `kclaw sessions`:
...
# Filter by platform
kclaw sessions list --source telegram

# Show more sessions
kclaw sessions list --limit 50
```
...
kclaw sessions export backup.jsonl

# Export sessions from a specific platform
kclaw sessions export telegram-history.jsonl --source telegram

# Export a single session
kclaw sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
kclaw sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
kclaw sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
kclaw sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
kclaw sessions prune

# Custom age threshold
kclaw sessions prune --older-than 30

# Only prune sessions from a specific platform
kclaw sessions prune --source telegram --older-than 60

# Skip confirmation
kclaw sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`kclaw insights`](/docs/reference/cli-commands#kclaw-insights).
...
When KClaw cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, KClaw uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to KClaw in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.kclaw/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.kclaw/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.kclaw/sessions/sessions.json` | Maps session keys to active session IDs |
...
kclaw sessions delete <session_id>
