...
title: "Git Worktrees"
description: "Run multiple KClaw agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

KClaw Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with KClaw so each session has a clean, isolated working directory.

## Why Use Worktrees with KClaw?

KClaw treats the **current working directory** as the project root:

- CLI: the directory where you run `kclaw` or `kclaw chat`
...
git worktree add ../repo-feature feature/kclaw-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/kclaw-experiment` checked out in that directory

Now you can `cd` into the new worktree and run KClaw there:

```bash
cd ../repo-feature

# Start KClaw in the worktree
kclaw
```
...
git worktree add ../repo-experiment-a feature/kclaw-a
git worktree add ../repo-experiment-b feature/kclaw-b
```
...
- Works on its own branch (`feature/kclaw-a` vs `feature/kclaw-b`).
...
- KClaw checkpoint data under `~/.kclaw/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per KClaw experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/kclaw-checkpoints-docs`, `feature/kclaw-refactor-tests`.
- **Commit frequently**
...
- **Avoid running KClaw from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `kclaw -w` (Automatic Worktree Mode)

KClaw has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `kclaw/kclaw-<hash>`).
...
```bash
kclaw -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `kclaw -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each KClaw session its own clean checkout.
