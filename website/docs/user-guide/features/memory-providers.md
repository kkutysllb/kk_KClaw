...
KClaw Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
kclaw memory setup      # interactive picker + configuration
kclaw memory status     # check what's active
kclaw memory off        # disable external provider
```

Or set manually in `~/.kclaw/config.yaml`:
...
When a memory provider is active, KClaw automatically:
...
**Setup Wizard:**
```bash
kclaw honcho setup        # (legacy command) 
# or
...
:::tip Migrating from `kclaw honcho`
If you previously used `kclaw honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each KClaw profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
kclaw profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `kclaw.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
kclaw honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `kclaw` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/kclaw-ai/kclaw/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kclaw).
...
kclaw memory setup    # select "openviking"
# Or manually:
...
**Setup:**
```bash
kclaw memory setup    # select "mem0"
# Or manually:
kclaw config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.kclaw/.env
```

**Config:** `$KCLAW_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `kclaw-user` | User identifier |
| `agent_id` | `kclaw` | Agent identifier |
...
**Setup:**
```bash
kclaw memory setup    # select "hindsight"
# Or manually:
...
**Local mode UI:** `hindsight-embed -p kclaw ui start`

**Config:** `$KCLAW_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `kclaw` | Memory bank identifier |
...
**Setup:**
```bash
kclaw memory setup    # select "holographic"
# Or manually:
kclaw config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.kclaw-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KCLAW_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
kclaw memory setup    # select "retaindb"
# Or manually:
...
# Then configure KClaw
kclaw memory setup    # select "byterover"
# Or manually:
...
- Knowledge tree stored at `$KCLAW_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
kclaw memory setup    # select "supermemory"
# Or manually:
kclaw config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.kclaw/.env
```

**Config:** `$KCLAW_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kclaw` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `kclaw-{identity}` → `kclaw-coder`) to isolate memories per KClaw profile
...
- **Local storage providers** (Holographic, ByteRover) use `$KCLAW_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$KCLAW_HOME/` so each profile has its own credentials
