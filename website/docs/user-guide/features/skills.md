...
All skills live in **`~/.kclaw/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point KClaw at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example of a skill-backed slash command with custom behavior. Running `/plan [request]` tells KClaw to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.kclaw/plans/` relative to the active workspace/backend working directory.
...
When a missing value is encountered, KClaw asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `kclaw setup` or `~/.kclaw/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `kclaw config migrate` prompts for unconfigured settings, and `kclaw config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.kclaw/skills/                  # Single source of truth
...
If you maintain skills outside of KClaw — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell KClaw to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.kclaw/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.kclaw/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, KClaw ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.kclaw/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
```bash
kclaw skills browse                              # Browse all hub skills (official first)
kclaw skills browse --source official            # Browse only official optional skills
kclaw skills search kubernetes                   # Search all sources
kclaw skills search react --source skills-sh     # Search the skills.sh directory
kclaw skills search https://mintlify.com/docs --source well-known
kclaw skills inspect openai/skills/k8s           # Preview before installing
kclaw skills install openai/skills/k8s           # Install with security scan
...
kclaw skills list --source hub                   # List hub-installed skills
kclaw skills check                               # Check installed hub skills for upstream updates
kclaw skills update                              # Reinstall hub skills with upstream changes when needed
kclaw skills audit                               # Re-scan all hub skills for security
kclaw skills uninstall k8s                       # Remove a hub skill
kclaw skills publish skills/my-skill --to github --repo owner/repo
kclaw skills snapshot export setup.json          # Export skill config
kclaw skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with KClaw. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `kclaw skills search <query> --source skills-sh`. KClaw resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
KClaw currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the KClaw repository itself and install with builtin trust.
...
This is Vercel's public skills directory. KClaw can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
KClaw can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
- KClaw source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

KClaw supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

KClaw source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

KClaw can search and convert agent entries from LobeHub's public catalog into installable KClaw skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- KClaw source id: `lobehub`
...
`kclaw skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with KClaw | Always trusted |
...
```bash
kclaw skills check          # Report which installed hub skills changed upstream
kclaw skills update         # Reinstall only the skills with updates available
kclaw skills update react   # Update one specific installed hub skill
```
