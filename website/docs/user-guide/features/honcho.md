...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of KClaw's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple KClaw instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
kclaw memory setup    # select "honcho" from the provider list
```
...
```bash
kclaw honcho status          # Show connection status and config
kclaw honcho peer            # Update peer names for multi-agent setups
```

## Migrating from `kclaw honcho`

If you previously used the standalone `kclaw honcho setup`:
...
No re-login or re-setup needed. Run `kclaw memory setup` and select "honcho" — the wizard detects your existing config.
