...
description: "Connect KClaw Agent to external tool servers via MCP — and control exactly which MCP tools KClaw loads"
---

# MCP (Model Context Protocol)

MCP lets KClaw Agent connect to external tool servers so the agent can use tools that live outside KClaw itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted KClaw to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native KClaw tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want KClaw to see
...
2. Add an MCP server to `~/.kclaw/config.yaml`:
...
4. Ask KClaw to use the MCP-backed capability.
...
KClaw will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints KClaw connects to directly.
...
- you do not want KClaw spawning a local subprocess for that integration

## Basic configuration reference

KClaw reads MCP config from `~/.kclaw/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, KClaw skips the server entirely |
...
## How KClaw registers MCP tools

KClaw prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — KClaw sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, KClaw also registers utility tools around MCP resources and prompts:
...
- KClaw only registers resource utilities if the MCP session actually supports resource operations
- KClaw only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to KClaw, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, KClaw skips the server completely and does not even attempt a connection.
...
You can also separately disable KClaw-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, KClaw does not create an empty runtime MCP toolset for that server.
...
KClaw discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify KClaw when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When KClaw receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, KClaw does not blindly pass your full shell environment.
...
Then verify your config and restart KClaw.
...
Because KClaw now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from KClaw via the `sampling/createMessage` protocol. This allows an MCP server to ask KClaw to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running KClaw as an MCP server

In addition to connecting **to** MCP servers, KClaw can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use KClaw's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through KClaw
- You want a single MCP server that bridges to all of KClaw's connected messaging platforms at once
- You already have a running KClaw gateway with connected platforms
...
Add KClaw to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed KClaw in a specific location:

```json
{
  "mcpServers": {
    "kclaw": {
      "command": "/home/user/.kclaw/kclaw/venv/bin/kclaw",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a KClaw-specific channel browser:
...
The MCP server includes a live event bridge that polls KClaw's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
kclaw mcp serve              # Normal mode
kclaw mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from KClaw's session store (`~/.kclaw/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the KClaw agent itself.
