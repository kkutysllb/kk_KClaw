...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with KClaw Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

KClaw Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with KClaw](/docs/guides/use-voice-mode-with-kclaw).

## Prerequisites

Before using voice features, make sure you have:

1. **KClaw Agent installed** — `pip install kclaw` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `kclaw model` or set your preferred provider credentials in `~/.kclaw/.env`
3. **A working base setup** — run `kclaw` to verify the agent responds to text before enabling voice

:::tip
The `~/.kclaw/` directory and default `config.yaml` are created automatically the first time you run `kclaw`. You only need to create `~/.kclaw/.env` manually for API keys.
:::
...
```bash
kclaw                # Start the interactive CLI
```
...
1. Start the CLI with `kclaw` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.kclaw/config.yaml` (default: `ctrl+b`).
:::
...
```bash
kclaw gateway        # Start the gateway (connects to configured platforms)
kclaw gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@kclawbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.kclaw/.env`:
```bash
...
```bash
kclaw gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.kclaw/.env`
...
- Review gateway logs: `tail -f ~/.kclaw/logs/gateway.log`
