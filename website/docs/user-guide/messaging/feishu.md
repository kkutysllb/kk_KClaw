...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up KClaw Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

KClaw Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; KClaw opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | KClaw responds to every message. |
| Group chats | KClaw responds only when the bot is @mentioned in the chat. |
...
Use WebSocket mode when KClaw runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run KClaw behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, KClaw starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how KClaw responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | KClaw responds to @mentions from any user in any group. |
| `allowlist` | KClaw only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | KClaw ignores all group messages entirely. |
...
|---------|---------|---------|
| Quiet period | `KCLAW_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `KCLAW_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `KCLAW_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `KCLAW_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.kclaw/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `KCLAW_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `KCLAW_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `KCLAW_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `KCLAW_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `KCLAW_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `KCLAW_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `kclaw gateway setup` |
| `Another local KClaw gateway is already using this Feishu app_id` | Only one KClaw instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `kclaw-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
