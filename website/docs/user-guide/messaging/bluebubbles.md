...
Connect KClaw to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between KClaw and the BlueBubbles server
...
Or set environment variables directly in `~/.kclaw/.env`:
...
When someone messages your iMessage, KClaw automatically sends them a pairing code. Approve it with:
```bash
kclaw pairing approve bluebubbles <CODE>
```
Use `kclaw pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.kclaw/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.kclaw/.env`):
```bash
...
KClaw will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → KClaw
KClaw → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** KClaw sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — KClaw resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `kclaw gateway logs` for webhook errors
