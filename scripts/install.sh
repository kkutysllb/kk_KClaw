...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/kclaw.git"
REPO_URL_HTTPS="https://github.com/NousResearch/kclaw.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.kclaw/kclaw)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/kclaw/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$KCLAW_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found (KClaw-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.kclaw/node/..."
    ...
    log_success "Node.js $installed_ver installed to ~/.kclaw/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "KClaw Agent itself does not require or retain root access."
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if KClaw behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up kclaw command..."
    ...
    if [ "$USE_VENV" = true ]; then
        ...
        if [ -z "$KCLAW_BIN" ]; then
            log_warn "kclaw not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$KCLAW_BIN" ]; then
        log_warn "kclaw entry point not found at $KCLAW_BIN"
    ...
    log_success "Symlinked kclaw → ~/.local/bin/kclaw"
    ...
    log_success "kclaw command ready"
}
...
copy_config_templates() {
    ...
    if [ ! -f "$KCLAW_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            ...
            log_success "Created ~/.kclaw/.env from template"
        else
            touch "$KCLAW_HOME/.env"
            log_success "Created ~/.kclaw/.env"
        fi
    else
        log_info "~/.kclaw/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$KCLAW_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            ...
            log_success "Created ~/.kclaw/config.yaml from template"
        fi
    else
        log_info "~/.kclaw/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.kclaw/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.kclaw/"

    # Seed bundled skills into ~/.kclaw/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.kclaw/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.kclaw/skills/"
    else
        ...
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$KCLAW_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            ...
            log_success "Skills copied to ~/.kclaw/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — KClaw itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'kclaw setup' after install."
        return 0
    fi
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for KClaw to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'kclaw whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'kclaw whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'kclaw gateway install' later."
        return 0
    fi
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $KCLAW_CMD gateway install 2>/dev/null; then
                ...
                if $KCLAW_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: kclaw gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: kclaw gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $KCLAW_CMD gateway > "$KCLAW_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.kclaw/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: kclaw gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: kclaw gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.kclaw/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.kclaw/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.kclaw/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.kclaw/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.kclaw/kclaw/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}kclaw${NC}              Start chatting"
    echo -e "   ${GREEN}kclaw setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}kclaw config${NC}       View/edit configuration"
    echo -e "   ${GREEN}kclaw config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}kclaw gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}kclaw update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'kclaw' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
