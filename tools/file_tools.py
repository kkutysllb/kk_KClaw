...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── KClaw internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from kclaw_constants import get_kclaw_home as _get_hh
        _kclaw_home = _get_hh().resolve()
        _blocked_dirs = [
            _kclaw_home / "skills" / ".hub" / "index-cache",
            _kclaw_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal KClaw cache file "
