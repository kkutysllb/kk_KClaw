...
- smoke-test a server with the FastMCP CLI before wiring it into KClaw or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to KClaw. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
When the goal is KClaw integration, either:

- configure the server in `~/.kclaw/config.yaml` using the `native-mcp` skill, or
...
### KClaw cannot see the deployed server

The server-building part may be correct while the KClaw config is not. Load the `native-mcp` skill and configure the server in `~/.kclaw/config.yaml`, then restart KClaw.
