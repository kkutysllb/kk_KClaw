...
# Uses get_kclaw_home() so each profile gets its own Matrix store.
from kclaw_constants import get_kclaw_dir as _get_kclaw_dir
_STORE_DIR = _get_kclaw_dir("platforms/matrix/store", "matrix/store")
...
_KEY_EXPORT_PASSPHRASE = "kclaw-matrix-e2ee-keys"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _run_e2ee_maintenance(self) -> None:
        ...
        KClaw uses a custom sync loop instead of matrix-nio's sync_forever(),
    ...
    def _thread_state_path() -> Path:
        ...
        return get_kclaw_home() / "matrix_threads.json"
