"""Anthropic Messages API adapter for KClaw Agent.

Translates between KClaw's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    KClaw historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by KClaw)
    ...
    # 1. KClaw-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration KClaw configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── KClaw-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.kclaw/.anthropic_oauth.json (our own file).
...
def run_kclaw_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run KClaw-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize KClaw with your Claude Pro/Max subscription.")
    print()
...
def _save_kclaw_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.kclaw/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        ...
        logger.debug("Failed to save KClaw OAuth credentials: %s", e)


def read_kclaw_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read KClaw-managed OAuth credentials from ~/.kclaw/.anthropic_oauth.json."""
    if _KCLAW_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read KClaw OAuth credentials: %s", e)
    return None
