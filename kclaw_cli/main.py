...
def _apply_profile_override() -> None:
    ...
    # 2. If no flag, check ~/.kclaw/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".kclaw" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set KCLAW_HOME
    if profile_name is not None:
        try:
            from kclaw_cli.profiles import resolve_profile_env
            kclaw_home = resolve_profile_env(profile_name)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'kclaw sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like KClaw isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  kclaw setup")
        print()

        from kclaw_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'kclaw setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from kclaw_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["KCLAW_SESSION_SOURCE"] = args.source
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with KClaw?")
        print()
    ...
    session_dir = get_kclaw_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: kclaw gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  kclaw gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ KClaw Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kclaw gateway")
        ...
        print()
        print("  Or install as a service: kclaw gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'kclaw whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    from kclaw_cli.config import load_config, get_env_value
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from kclaw_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from kclaw_cli.config import get_env_value, save_config, save_env_value
    from kclaw_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from kclaw_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `kclaw auth` stores device_code tokens),
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from kclaw_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    from kclaw_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `kclaw model` to set a model.")
...
from kclaw_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from kclaw_cli.config import save_env_value, load_config, save_config
    from kclaw_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from kclaw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from kclaw_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates KClaw turns to `copilot --acp`.")
    print("  KClaw currently starts its own ACP subprocess for each request.")
    print("  KClaw uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set KCLAW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from kclaw_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from kclaw_cli.config import get_env_value, save_env_value, load_config, save_config
    from kclaw_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    KClaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'kclaw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_version(args):
    ...
    try:
        from kclaw_cli.banner import check_for_updates
...
def _update_via_zip(args):
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/kclaw/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="kclaw-update-")
        zip_path = os.path.join(tmp_dir, f"kclaw-{branch}.zip")
        ...
        # GitHub ZIPs extract to kclaw-<branch>/
        extracted = os.path.join(tmp_dir, f"kclaw-{branch}")
...
# =========================================================================
# Fork detection and upstream management for `kclaw update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/kclaw.git",
    "git@github.com:NousResearch/kclaw.git",
    "https://github.com/NousResearch/kclaw",
    "git@github.com:NousResearch/kclaw",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/kclaw.git"
...
def _invalidate_update_cache():
    ...
    default_home = Path.home() / ".kclaw"
    homes.append(default_home)
    # Named profiles under ~/.kclaw/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "kclaw[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    ...
    if is_managed():
        managed_error("update KClaw Agent")
        return
    ...
    try:
        ...
        # attributes like display_kclaw_home() added since the last release.
        try:
            import importlib
            import kclaw_constants as _hc
        ...
        try:
            from kclaw_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        try:
            ...
            # Discover all kclaw-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "kclaw-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. kclaw-gateway.service or kclaw-gateway-coder.service
            ...
            if is_macos():
                try:
                    from kclaw_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → kclaw -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kclaw)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  kclaw profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     kclaw -p {name} chat")
                else:
            ...
            if clone or clone_all:
                profile_dir_display = f"~/.kclaw/profiles/{name}"
        ...
        name = args.profile_name
        from kclaw_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from kclaw_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from kclaw_cli.profiles import rename_profile
        try:
        ...
        from kclaw_cli.profiles import export_profile
        name = args.profile_name
        ...
        from kclaw_cli.profiles import import_profile
        try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    kclaw                        Start interactive chat
    kclaw chat -q "Hello"        Single query mode
    kclaw -c                     Resume the most recent session
    kclaw -c "my project"        Resume a session by name (latest in lineage)
    kclaw --resume <session_id>  Resume a specific session by ID
    kclaw setup                  Run setup wizard
    kclaw logout                 Clear stored authentication
    kclaw auth add <provider>    Add a pooled credential
    kclaw auth list              List pooled credentials
    kclaw auth remove <p> <t>    Remove pooled credential by index, id, or label
    kclaw auth reset <provider>  Clear exhaustion status for a provider
    kclaw model                  Select default model
    kclaw config                 View configuration
    kclaw config edit            Edit config in $EDITOR
    kclaw config set model gpt-4 Set a config value
    kclaw gateway                Run messaging gateway
    kclaw -s kclaw-dev,github-auth
    kclaw -w                     Start in isolated git worktree
    kclaw gateway install        Install gateway background service
    kclaw sessions list          List past sessions
    kclaw sessions browse        Interactive session picker
    kclaw sessions rename ID T   Rename/title a session
    kclaw logs                   View agent.log (last 50 lines)
    kclaw logs -f                Follow agent.log in real time
    kclaw logs errors            View errors.log
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with KClaw Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: kclaw-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure KClaw Agent with an interactive wizard. "
                    "Run a specific section: kclaw setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for KClaw CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: kclaw-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of KClaw Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with KClaw Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your KClaw setup "
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage KClaw Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from kclaw_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from kclaw_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from kclaw_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/kclaw-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from kclaw_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from kclaw_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'kclaw tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # kclaw tools disable <name...> [--platform cli]
    ...
    # kclaw tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            _require_tty("tools")
            from kclaw_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run KClaw as an MCP server",
        description=(
            "Manage MCP server connections and run KClaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'kclaw mcp add' to connect to a new server, or\n"
            "'kclaw mcp serve' to expose KClaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run KClaw as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from kclaw_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to KClaw"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to KClaw",
    ...
    def cmd_claw(args):
        from kclaw_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update KClaw Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall KClaw Agent",
        description="Remove KClaw Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run KClaw Agent as an ACP (Agent Client Protocol) server",
        description="Start KClaw Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated KClaw instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter KClaw log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kclaw logs                    Show last 50 lines of agent.log
    kclaw logs -f                 Follow agent.log in real time
    kclaw logs errors             Show last 50 lines of errors.log
    kclaw logs gateway -n 100     Show last 100 lines of gateway.log
    kclaw logs --level WARNING    Only show WARNING and above
    kclaw logs --session abc123   Filter by session ID
    kclaw logs --since 1h         Lines from the last hour
    kclaw logs --since 30m -f     Follow, starting from 30 min ago
    kclaw logs list               List available log files with sizes
""",
    )
