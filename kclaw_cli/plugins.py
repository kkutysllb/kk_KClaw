...
ENTRY_POINTS_GROUP = "kclaw_agent.plugins"

_NS_PARENT = "kclaw_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``kclaw honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.kclaw/plugins/)
        user_dir = get_kclaw_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.kclaw/plugins/)
        if _env_enabled("KCLAW_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".kclaw" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``kclaw_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``kclaw tools`` TUI so plugin-provided toolsets appear
