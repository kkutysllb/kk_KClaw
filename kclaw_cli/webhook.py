"""kclaw webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.kclaw/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: kclaw gateway run
"""
...
def webhook_command(args):
    """Entry point for 'kclaw webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: kclaw webhook {subscribe|list|remove|test}")
        print("Run 'kclaw webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (kclaw gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: kclaw webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (kclaw gateway run)")
