...
KCLAW_OVERLAYS: Dict[str, KClawOverlay] = {
    "openrouter": KClawOverlay(
    ...
    "openai-codex": KClawOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": KClawOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="KCLAW_QWEN_BASE_URL",
    ),
    "copilot-acp": KClawOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": KClawOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": KClawOverlay(
    ...
    "kimi-for-coding": KClawOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": KClawOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": KClawOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": KClawOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": KClawOverlay(
        transport="openai_chat",
    ...
    "opencode": KClawOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": KClawOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kclaw", "user-config"
...
def get_overlay(provider_id: str) -> Optional[KClawOverlay]:
    """Get KClaw overlay for a provider, if one exists."""
    canonical = normalize_provider(provider_id)
    return KCLAW_OVERLAYS.get(canonical)
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. KClaw overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + KClaw overlay
    ...
    overlay = KCLAW_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + kclaw extra
    ...
    if overlay is not None:
        # KClaw-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy KClaw IDs (point to same providers)
