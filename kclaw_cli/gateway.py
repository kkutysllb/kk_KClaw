...
from kclaw_cli.config import get_env_value, get_kclaw_home, save_env_value, is_managed, managed_error
# display_kclaw_home is imported lazily at call sites to avoid ImportError
# when kclaw_constants is cached from a pre-update version during `kclaw update`.
from kclaw_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "kclaw-gateway*",
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KCLAW_HOME-scoped).
...
_SERVICE_BASE = "kclaw-gateway"
SERVICE_DESCRIPTION = "KClaw Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KCLAW_HOME.

    Returns ``""`` for the default ``~/.kclaw``, the profile name for
    ``~/.kclaw/profiles/<name>``, or a short hash for any other custom
    KCLAW_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_kclaw_home().resolve()
    default = (_Path.home() / ".kclaw").resolve()
    if home == default:
        return ""
    # Detect ~/.kclaw/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KCLAW_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(kclaw_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when KCLAW_HOME is a named profile.

    For ``~/.kclaw/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        kclaw_home: Optional explicit KCLAW_HOME path. Defaults to the current
            ``get_kclaw_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from pathlib import Path as _Path
    home = Path(kclaw_home or str(get_kclaw_home())).resolve()
    default = (_Path.home() / ".kclaw").resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this KCLAW_HOME.

    Default ``~/.kclaw`` returns ``kclaw-gateway`` (backward compatible).
    Profile ``~/.kclaw/profiles/coder`` returns ``kclaw-gateway-coder``.
    Any other KCLAW_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    kclaw gateway uninstall")
    print_info("    sudo kclaw gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so KClaw can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo kclaw gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo kclaw gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo kclaw gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kclaw`` → ``ai.kclaw.gateway.plist`` (backward compatible).
    Profile ``~/.kclaw/profiles/coder`` → ``ai.kclaw.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.kclaw.gateway-{suffix}" if suffix else "ai.kclaw.gateway"
...
def get_kclaw_cli_path() -> str:
    """Get the path to the kclaw CLI."""
    ...
    return f"{get_python_path()} -m kclaw_cli.main"
...
def _kclaw_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current KCLAW_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_kclaw_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.kclaw                    → /home/alice/.kclaw
      /root/.kclaw/profiles/coder     → /home/alice/.kclaw/profiles/coder
      /opt/custom-kclaw               → /opt/custom-kclaw  (kept as-is)
    """
    current_kclaw = get_kclaw_home().resolve()
    current_default = (Path.home() / ".kclaw").resolve()
    target_default = Path(target_home_dir) / ".kclaw"

    # Default ~/.kclaw → remap to target user's default
    if current_kclaw == current_default:
        return str(target_default)

    # Profile or subdir of ~/.kclaw → preserve the relative structure
    try:
        relative = current_kclaw.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.kclaw) — keep as-is
        return str(current_kclaw)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        kclaw_home = _kclaw_home_for_target_user(home_dir)
        profile_arg = _profile_arg(kclaw_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m kclaw_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="KCLAW_HOME={kclaw_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    kclaw_home = str(get_kclaw_home().resolve())
    profile_arg = _profile_arg(kclaw_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current KClaw install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}kclaw gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}kclaw gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}kclaw gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}kclaw gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}kclaw gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.kclaw.gateway-{suffix}" if suffix else "ai.kclaw.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current KClaw install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  kclaw gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current KClaw install")
    else:
        print("⚠ Service definition is stale relative to the current KClaw install")
        print("  Run: kclaw gateway start")
    ...
    if deep:
        log_file = get_kclaw_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and KClaw will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @kclaw:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. kclaw) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where KClaw delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your KClaw agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address KClaw will use (e.g., kclaw@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. KClaw connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: kclaw pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_kclaw_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'kclaw pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: kclaw pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'kclaw gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from kclaw_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"KClawAgent\"")
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: kclaw gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service? (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: kclaw gateway install")
                else:
                    print_info("  You can install later: kclaw gateway install")
                    if is_linux():
                        print_info("  Or as a boot-time service: sudo kclaw gateway install --system")
                    print_info("  Or run in foreground:  kclaw gateway")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: kclaw gateway")
    else:
        print()
        print_info("No platforms configured. Run 'kclaw gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: kclaw gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: kclaw gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  kclaw gateway install")
                print("  sudo kclaw gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  kclaw gateway          # Run in foreground")
                print("  kclaw gateway install  # Install as user service")
                print("  sudo kclaw gateway install --system  # Install as boot-time system service")
