...
KCLAW_CADUCEUS = """[bold #FFD700]                                    [/]
[bold #FFD700]             _oo0oo_              [/]
[bold #FFD700]            o88888888o            [/]
[bold #FFD700]            88"  .  "88           [/]
[bold #FFD700]            (| -_- |)            [/]
[bold #FFBF00]            0\  =  /0            [/]
[bold #FFBF00]          ___/`---'\___          [/]
...
[bold #B8860B]     \  .-\__  `-`  ___/-. /    [/]
[bold #B8860B]   ___`. .'  /--.--\  `. .'___  [/]
[bold #B8860B] .\"""< `.___\\<|>_/___.' >\""".[/]
[bold #B8860B] | | :  `- \`.;`\ _ /`;.`/ - ` : | | [/]
[bold #B8860B] \  \ `_.   \_ __\ /__ _/   .-` /  / [/]
[bold #B8860B] =====`-.____`.___ \_____/___.-`____.-'===== [/]
[bold #B8860B]               `=---='               [/]"""

COMPACT_BANNER = """
[bold #FFD700]╔══════════════════════════════════════════════════════════════╗[/]
[bold #FFD700]║[/]  [#FFBF00]⚕ KCLAW Super Agent[/] [dim #B8860B]- AI Agent Framework[/]          [bold #FFD700]║[/]
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.kclaw/.update_check``).  Returns the number of commits behind,
    or ``None`` if the check fails or isn't applicable.
    """
    kclaw_home = get_kclaw_home()
    repo_dir = kclaw_home / "kclaw"
    cache_file = kclaw_home / ".update_check"
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active KClaw git checkout, or None if this isn't a git install."""
    kclaw_home = get_kclaw_home()
    repo_dir = kclaw_home / "kclaw"
    if not (repo_dir / ".git").exists():
        repo_dir = Path(__file__).parent.parent.resolve()
...
def format_banner_version_label() -> str:
    ...
    base = f"KClaw Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else KCLAW_CADUCEUS
    except Exception:
        _bskin = None
        _hero = KCLAW_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]kkutysllb[/]")
    ...
    if skills_by_category:
        for category in sorted(skills_by_category.keys()):
            ...
            if len(skill_names) > 8:
                ...
                skills_str = ", ".join(display_names) + f" 等{len(skill_names) - 8}个"
            else:
        ...
        right_lines.append(f"[dim {dim}]暂无已安装的技能[/]")

    right_lines.append("")
    mcp_connected = sum(1 for s in mcp_status if s["connected"]) if mcp_status else 0
    summary_parts = [f"{len(tools)} 个工具", f"{total_skills} 个技能"]
    if mcp_connected:
        summary_parts.append(f"{mcp_connected} 个MCP服务")
    ...
    try:
        ...
        if _profile_name and _profile_name != "default":
            right_lines.append(f"[bold {accent}]当前档案:[/] [{text}]{_profile_name}[/]")
    except Exception:
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            right_lines.append(
                ...
                f"[dim yellow] — 运行 [bold]{recommended_update_command()}[/bold] 更新[/]"
            )
    except Exception:
    ...
    agent_name = _skin_branding("agent_name", "KClaw Agent")
