...
from kclaw_cli.providers import (
    determine_api_mode,
    get_label,
    is_aggregator,
    resolve_provider_full,
)
from kclaw_cli.model_normalize import (
...
def _check_kclaw_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a KClaw LLM model."""
    if "kclaw" in model_name.lower():
        return _KCLAW_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kclaw model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kclaw doctor' — config issues detected:"
    ...
    kclaw_warn = _check_kclaw_model_warning(new_model)
    if kclaw_warn:
        warnings.append(kclaw_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from kclaw_cli/models.py (OPENROUTER_MODELS,
    ...
    from kclaw_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by kclaw provider ID
    ...
    # --- 1. Check KClaw-mapped providers ---
    for kclaw_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        model_ids = curated.get(kclaw_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check KClaw-only providers (nous, openai-codex, copilot) ---
    from kclaw_cli.providers import KCLAW_OVERLAYS
    for pid, overlay in KCLAW_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external", "external_process"):
            ...
            try:
                from kclaw_cli.auth import _load_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: kclaw setup -> Custom OpenAI-compatible endpoint"
            ),
        )
