...
# Directories/files to exclude when exporting the default (~/.kclaw) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kclaw",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "kclaw", "default", "test", "tmp", "root", "sudo",
})

# KClaw subcommands that cannot be used as profile names/aliases
_KCLAW_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.kclaw/profiles/`` — anchored to the user's home,
    NOT to the current KCLAW_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".kclaw" / "profiles"


def _get_default_kclaw_home() -> Path:
    """Return the default (pre-profile) KCLAW_HOME path."""
    return Path.home() / ".kclaw"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_kclaw_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.kclaw
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KCLAW_HOME directory."""
    if name == "default":
        return _get_default_kclaw_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, kclaw subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _KCLAW_SUBCOMMANDS:
        return f"'{name}' conflicts with a kclaw subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kclaw_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kclaw)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KCLAW_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KCLAW_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kclaw).\n"
            "To remove everything, use: kclaw uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KCLAW_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KCLAW_HOME")
    try:
        os.environ["KCLAW_HOME"] = str(profile_dir)
        from kclaw_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KCLAW_HOME"] = old_home
        elif "KCLAW_HOME" in os.environ:
            del os.environ["KCLAW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kclaw/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: kclaw profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KCLAW_HOME.

    Returns ``"default"`` if KCLAW_HOME is not set or points to ``~/.kclaw``.
    Returns the profile name if KCLAW_HOME points into ``~/.kclaw/profiles/<name>``.
    Returns ``"custom"`` if KCLAW_HOME is set to an unrecognized path.
    """
    from kclaw_constants import get_kclaw_home
    kclaw_home = get_kclaw_home()
    resolved = kclaw_home.resolve()

    default_resolved = _get_default_kclaw_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.kclaw itself — its parent is ~/ and its
        # directory name is ".kclaw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kclaw profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.kclaw itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kclaw). "
            "Specify a different name: kclaw profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for kclaw profile names."""
    return '''# KClaw Agent profile completion
# Add to ~/.bashrc: eval "$(kclaw completion bash)"

_kclaw_profiles() {
    local profiles_dir="$HOME/.kclaw/profiles"
...
_kclaw_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_kclaw_profiles)" -- "$cur"))
        return
    fi
...
complete -F _kclaw_completion kclaw
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for kclaw profile names."""
    return '''#compdef kclaw
# KClaw Agent profile completion
# Add to ~/.zshrc: eval "$(kclaw completion zsh)"

_kclaw() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kclaw/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.kclaw/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KCLAW_HOME path string.

    Called early in the CLI entry point, before any kclaw modules
    are imported, to set the KCLAW_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: kclaw profile create {profile_name}"
        )
