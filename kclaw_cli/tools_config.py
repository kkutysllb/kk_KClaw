...
# They're still in _KCLAW_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from kclaw_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "kclaw-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "kclaw-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "kclaw-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "kclaw-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "kclaw-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "kclaw-signal"},
    "bluebubbles": {"label": "💙 BlueBubbles", "default_toolset": "kclaw-bluebubbles"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "kclaw-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "kclaw-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "kclaw-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "kclaw-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "kclaw-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "kclaw-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "kclaw-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "kclaw-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "kclaw-webhook"},
}
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kclaw_home()}/kclaw && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kclaw directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "kclaw-cli" (which include all _KCLAW_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "kclaw-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `kclaw tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by kclaw tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kclaw-cli, kclaw-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.kclaw/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `kclaw tools` and `kclaw setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ KClaw Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://kclaw.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from kclaw_constants import display_kclaw_home
    print(color(f"  Tool configuration saved to {display_kclaw_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'kclaw' or gateway restart.", Colors.DIM))
    print()
