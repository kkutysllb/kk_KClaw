...
from kclaw_cli.config import get_kclaw_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_kclaw.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_kclaw", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: kclaw claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to KClaw")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'kclaw claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → KClaw migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: kclaw claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {kclaw_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: kclaw claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  kclaw claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  kclaw claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  kclaw config set OPENROUTER_API_KEY sk-or-v1-...")
