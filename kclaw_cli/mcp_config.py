...
def cmd_mcp_add(args):
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kclaw_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: kclaw mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_kclaw_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kclaw mcp add <name> --url <endpoint>')
        _info('  kclaw mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'kclaw mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``kclaw mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("kclaw mcp serve                              Run as MCP server")
        _info("kclaw mcp add <name> --url <endpoint>        Add an MCP server")
        _info("kclaw mcp add <name> --command <cmd>         Add a stdio server")
        _info("kclaw mcp remove <name>                      Remove a server")
        _info("kclaw mcp list                               List servers")
        _info("kclaw mcp test <name>                        Test connection")
        _info("kclaw mcp configure <name>                   Toggle tools")
        print()
