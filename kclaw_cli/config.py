...
from kclaw_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_kclaw_home() / ".managed"
...
def is_managed() -> bool:
    """Check if KClaw is running in package-manager-managed mode.

    Two signals: the KCLAW_MANAGED env var (set by the systemd service),
    or a .managed marker file in KCLAW_HOME (set by the NixOS activation
...
# Re-export from kclaw_constants — canonical definition lives there.
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when KClaw
    ...
    "browser": {
        ...
        "camofox": {
            # When true, KClaw sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.kclaw/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for kclaw-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *KClaw Agent*" header.
    ...
    # Logging — controls file logging to ~/.kclaw/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the KClaw auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kclaw:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KCLAW_BOT)",
    ...
    # KCLAW_TOOL_PROGRESS and KCLAW_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kclaw.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kclaw setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — KClaw won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kclaw doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("KCLAW_TOOL_PROGRESS")
            old_mode = get_env_value("KCLAW_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from KCLAW_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from KCLAW_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KCLAW_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KCLAW_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    # metadata.kclaw.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kclaw/config.yaml as-is, without merging defaults or migrating.
...
#   openai-codex (OAuth — kclaw auth) — OpenAI Codex
#   nous         (OAuth — kclaw auth) — Nous Portal
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real KClaw env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kclaw/.env in place.
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kclaw/.env and os.environ.
