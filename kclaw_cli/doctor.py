...
from kclaw_cli.config import get_project_root, get_kclaw_home, get_env_path
from kclaw_constants import display_kclaw_home

PROJECT_ROOT = get_project_root()
KCLAW_HOME = get_kclaw_home()
_DHH = display_kclaw_home()  # user-facing display path (e.g. ~/.kclaw or ~/.kclaw/profiles/coder)

# Load environment variables from ~/.kclaw/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.kclaw/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `kclaw`.
    os.environ.setdefault("KCLAW_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 KClaw Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.kclaw/.env (primary location for user config)
    env_path = KCLAW_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'kclaw setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'kclaw setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'kclaw setup' to create one")
                issues.append("Run 'kclaw setup' to create .env")
    
    # Check ~/.kclaw/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from kclaw_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'kclaw setup' to migrate config")
                else:
                    issues.append("Run 'kclaw doctor --fix' or 'kclaw setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'kclaw doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from kclaw_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    for subdir_name in expected_subdirs:
        subdir_path = kclaw_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give KClaw a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# KClaw Agent Persona\n\n"
                "<!-- Edit this file to customize how KClaw communicates. -->\n\n"
                "You are KClaw, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = kclaw_home / "memories"
    ...
    state_db_path = kclaw_home / "state.db"
    ...
    wal_path = kclaw_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'kclaw doctor --fix' to checkpoint")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/kclaw/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'kclaw setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: kclaw skills list)")
    ...
    try:
        ...
        _mem_cfg_path = KCLAW_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: kclaw memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: kclaw memory setup")
                issues.append("No Honcho API key — run 'kclaw memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run kclaw memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: kclaw memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: kclaw memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from kclaw_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "kclaw -p" in content:
                            _m = _re.search(r"kclaw -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'kclaw doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
