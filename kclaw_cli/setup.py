...
_DOCS_BASE = "https://kclaw.nousresearch.com/docs"
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from kclaw_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    from kclaw_cli.config import get_env_value
...
# display_kclaw_home imported lazily at call sites (stale-module safety during kclaw update)
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'kclaw setup')")
...
def _print_setup_summary(config: dict, kclaw_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'kclaw setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'kclaw setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'kclaw setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'kclaw setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {kclaw_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('kclaw setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('kclaw setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('kclaw setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('kclaw setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('kclaw setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('kclaw config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('kclaw config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('kclaw config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('kclaw', Colors.GREEN)}              Start chatting")
    print(f"   {color('kclaw gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('kclaw doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `kclaw tools` and `kclaw setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``kclaw model``)
    ...
    provider added to ``kclaw model`` is automatically available here.
    ...
    # Delegate to the shared kclaw model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from kclaw_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: kclaw model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from kclaw_cli.auth_commands import auth_add_command
            ...
            print_info(
                "KClaw can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one KClaw can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'kclaw setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.kclaw/.env."
            )
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where KClaw runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using KClaw via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `kclaw setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("KCLAW_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("KCLAW_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where KClaw delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://kclaw.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where KClaw delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where KClaw delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'kclaw whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'kclaw whatsapp' to choose your mode (separate bot number")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://kclaw.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://kclaw.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  kclaw config edit")
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'kclaw setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     kclaw config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            print_info("   kclaw gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `kclaw setup tools` and `kclaw tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from kclaw_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_kclaw.py"
)


def _offer_openclaw_migration(kclaw_home: Path) -> bool:
    ...
    print_info("KClaw can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_kclaw", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=kclaw_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in KClaw.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ KClaw Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ KClaw Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your KClaw Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have KClaw configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, kclaw_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'kclaw setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'kclaw setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, kclaw_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(kclaw_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up KClaw?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, kclaw_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {kclaw_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'kclaw config edit'")
...
def _run_first_time_quick_setup(config: dict, kclaw_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'kclaw setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    kclaw setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  kclaw setup gateway")
    print()
...
def _run_quick_setup(config: dict, kclaw_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'kclaw setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect KClaw to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'kclaw setup gateway'.")
